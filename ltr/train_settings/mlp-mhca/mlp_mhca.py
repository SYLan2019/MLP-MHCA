...
import ltr.models.tracking.mlp_mhca as mlp_mhca_models
...
def run(settings):
    ...
    settings.description = 'MLP-MHCA with default settings.'
    ...
    settings.search_sz = settings.search_feature_sz * 8      # 32*8=256
    settings.temp_sz = settings.template_feature_sz * 8      # 16*8=128
    ...
    data_processing_train = processing.MLP_MHCAProcessing(search_area_factor=settings.search_area_factor,
    ...
    dataset_train = sampler.MLP_MHCASampler([lasot_train, got10k_train, coco_train, trackingnet_train], [1,1,1,1],
    ...
    model = mlp_mhca_models.transt_resnet50(settings)

    # Wrap the network for multi GPU training
    if settings.multi_gpu:
        model = MultiGPU(model, dim=0)

    objective = mlp_mhca_models.train_loss(settings)
    n_parameters = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print('number of params:', n_parameters)

    actor = actors.MLP_MHCAActor(net=model, objective=objective)
